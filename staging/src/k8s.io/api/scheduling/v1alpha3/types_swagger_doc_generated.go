...
var map_PodGroupSchedulingPolicy = map[string]string{
	...
	"basic": "Basic specifies that the pods in this group should be scheduled using standard Kubernetes scheduling behavior. This field is immutable.",
	"gang":  "Gang specifies that the pods in this group should be scheduled using all-or-nothing semantics. This field cannot be set/unset during update.",
}
...
var map_PodGroupTemplate = map[string]string{
	...
	"resourceClaims":        "ResourceClaims defines which ResourceClaims may be shared among Pods in the group. Pods consume the devices allocated to a PodGroup's claim by defining a claim in its own Spec.ResourceClaims that matches the PodGroup's claim exactly. The claim must have the same name and refer to the same ResourceClaim or ResourceClaimTemplate.\n\nThis is an alpha-level field and requires that the DRAWorkloadResourceClaims feature gate is enabled.\n\nThis field is immutable.",
	"disruptionMode":        "DisruptionMode defines the mode in which a given PodGroup can be disrupted. One of Single, All. This field is available only when the WorkloadAwarePreemption feature gate is enabled. This field is immutable.",
...
var map_WorkloadSpec = map[string]string{
	...
	"podGroupTemplates": "PodGroupTemplates is the list of templates that make up the Workload. The maximum number of templates is 8. This field cannot have added/removed items during updates.",
}
