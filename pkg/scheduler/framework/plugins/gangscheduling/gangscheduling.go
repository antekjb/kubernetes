...
func (pl *GangScheduling) isSchedulableAfterPodGroupUpdated(logger klog.Logger, pod *v1.Pod, oldObj, newObj interface{}) (fwk.QueueingHint, error) {
	...
	if oldPodGroup == nil || newPodGroup == nil {
		return fwk.Queue, fmt.Errorf("unexpected nil PodGroup in update event: old=%p, new=%p", oldPodGroup, newPodGroup)
	}
	...
	// If the gang scheduling policy minCount has decreased, it might make the waiting pods schedulable.
	if oldPolicy.Gang != nil && newPolicy.Gang != nil && newPolicy.Gang.MinCount < oldPolicy.Gang.MinCount {
		logger.V(4).Info("PodGroup minCount decreased, enqueuing pod", "pod", klog.KObj(pod), "podGroup", klog.KObj(newPodGroup), "oldMinCount", oldPolicy.Gang.MinCount, "newMinCount", newPolicy.Gang.MinCount)
		return fwk.Queue, nil
	}
